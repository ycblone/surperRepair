<template>
  <div id="req2" style="width: 97%;">
    <el-badge is-dot class="item" style="margin-top: 2em;font-size: 0.15rem">直接申请配件</el-badge>
    <!--动态生成申请数据栏，得到每栏的数据源和下标。利用v-if来显示移除数据栏-->
    <div class="container-confirm" v-for="(part,index) in partsReq2" v-if="!showFlag[index]">
      ...
      <p style="text-align: left;margin-top: 0.5em">使用物业：<span>{{part.equipment.wy.name}}</span></p>
      <p style="text-align: left;margin-top: 0.5em">数量：<span>{{part.num}}</span></p>
      <p style="text-align: left;margin-top: 0.5em">维修设备：<span>{{part.equipment.name}}</span></p>
      <p style="text-align: left;margin-top: 0.5em">备注：<span>{{part.beizhu}}</span></p>
