<template>
  ...
  <div id="repairListFixer" style=" height:100vh;background-color: #eaeaea;">
    <van-row type="flex" justify="space-between" style="height: 67px;background-color: darkgoldenrod;color: whitesmoke;font-size: 20px;font-weight: bold;line-height: 67px;letter-spacing:4px;" class="header">
      ...
      <van-col span="8" offset="2">全部工单</van-col>
      <van-col span="6"></van-col>
    </van-row>

    <div class="listCon container-fluid" style="width: 93%;background-color: white;display: inline-block;text-align: center;border-radius: 5px;margin-top: 10px;font-size: 16px;padding: 20px 15px;color: #989898;" v-for="list in listData" @click="toSign(list)" v-if="list.state !== 0">
      <van-row type="flex">
